...
#    generated header below gives droidcheck the names to gate on — same
#    overlay file the engine's jack tables are generated from, so the two can
#    never disagree about what is experimental.
python3 - "$SRC/droidfirmware.json" "$HERE/../jackgen" "$BUILD/droidfirmware.json" \
          "$BUILD/experimental_names.h" <<'PY'
import json, sys
fw_path, jackgen_dir, out_json, out_hdr = sys.argv[1:5]
sys.path.insert(0, jackgen_dir)
import overlay          # tools/jackgen/overlay.py — shared with make gen

fw = json.load(open(fw_path))
# Same loader the engine's codegen uses, so the two can never disagree about
# which circuits exist or which are experimental (ADR 0001).
experimental = overlay.load(fw["circuits"])
names = [name for name, _ in experimental]
for name, circuit in experimental:
    fw["circuits"][name] = circuit
json.dump(fw, open(out_json, "w"))
with open(out_hdr, "w") as f:
    f.write("// GENERATED by tools/droidcheck/build.sh — DO NOT EDIT\n")
    f.write("static const char *kExperimentalCircuits[] = {\n")
    for name in names:
        f.write(f'    "{name}",\n')
    f.write("    nullptr\n};\n")
print(f"merged {len(names)} experimental circuit(s): {', '.join(names) or '(none)'}")
PY
